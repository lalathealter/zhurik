...
Проверить это можно при помощи команд `python3 -v` и `pip3 -v`, введёных в консоли - если они исполняются без ошибки, то всё в порядке. (ЗАМЕЧАНИЕ: Если по той или иной причине у вас не получается использовать python3 или pip3, то попробуйте ввести ту же самую команду, но без тройки в конце - авось получится)
...
Для правильной работы статистики необходимо настроить базу данных (на sqlite). Сделать это можно следующей командой: `python3 db_init.py`.
Замечание для будущих разработчиков: именно в этом файле задаётся структура базы данных в формате SQL. Если вы захотите её поменять, то не забудьте обновить её или в консоли соответствующими командами (вручную), или же запуском `python3 db_drop.py` (сбросом всей базы данных) и последующим `python3 db_init.py` (пересозданием).
